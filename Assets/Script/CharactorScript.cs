...
public partial class ControllerScript : MonoBehaviour
{
    ...
    public class WereWolf : Charactor
    {
        ...
        IEnumerator timer()
        {
            while (true)
            {
                if (showAttack)//if를 지워도 되지 않을까 어차피 클릭자체가 이벤트이자 if임 , if를 지우면 1.x초에서 클릭하면 잠깐 보이고 없어짐
                {
                    //ControllerScript.instance.meleeAttack.SetActive(true);
                    Debug.Log("보임");
    ...
    public class Human : Charactor
    {
        ...
        public override void Attack()
        {
            ...
            _bullet.GetComponent<BulletScript>().targetPos = ControllerScript.instance.ClickPos();
